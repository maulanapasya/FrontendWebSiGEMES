    ...
    <div class="container mt-6 mx-auto px-6 py-8">
      ...
      <div class="flex flex-wrap gap-4 mt-6">
        <div class="bg-blue-100 p-6 rounded-xl shadow-lg flex-1">
          <p class="text-gray-800">Pendapatan {{ loadingRevenue ? '...' : currentDayText }}</p>
          <h2 class="revenue text-3xl text-black font-bold">
            <span v-if="loadingRevenue" class="skeleton-loader">Rp0.000.000</span>
            <span v-else>{{ formatCurrency(dailyRevenue) }}</span>
          </h2>
        </div>
        <div class="bg-blue-100 p-6 rounded-xl shadow-lg flex-1">
          <p class="text-gray-800">Pendapatan bulan {{ loadingRevenue ? '...' : currentMonthText }}</p>
          <h2 class="revenue text-3xl text-black font-bold">
            <span v-if="loadingRevenue" class="skeleton-loader">Rp00.000.000</span>
            <span v-else>{{ formatCurrency(monthlyRevenue) }}</span>
          </h2>
        </div>
        <div class="bg-blue-100 p-6 rounded-xl shadow-lg flex-1">
          <p class="text-gray-800">Total pendapatan tahun {{ loadingRevenue ? '...' : currentYear }}</p>
          <h2 class="revenue text-3xl text-black font-bold">
            <span v-if="loadingRevenue" class="skeleton-loader">Rp00.000.000</span>
