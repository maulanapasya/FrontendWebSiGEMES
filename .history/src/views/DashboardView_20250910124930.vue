...
export default {
  ...
  methods: {
    ...
    // MODIFIED createExcelReport method - Creates individual sheets per building
    async createExcelReport(messTransactions, nationalTransactions, month, year) {
      ...
      // Create sheets for each selected guesthouse (Mess)
      for (const guesthouseId of this.selectedGuesthouses) {
        const guesthouse = this.guesthousesList.find(g => g.id === guesthouseId);
        const guesthouseTransactions = messTransactions.filter(rental => 
          (rental.payment?.status === 'paid' || rental.payment?.status === 'dibayar') &&
          rental.guesthouse_room_pricing?.retribution_type &&
          rental.guesthouse_room_pricing?.guesthouse_room?.guesthouse?.id === guesthouseId
        );
        ...
        const messData = guesthouseTransactions.map((rental, index) => [
          index + 1,
        ...
        // Add "Tidak ada data" row if no transactions
        if (messData.length === 0) {
          messSheetData.push(['', '', '', 'Tidak ada transaksi dalam periode ini', '', '', '', '', '']);
        }
        ...
        const sheetName = `Transaksi ${guesthouse?.name || `Mess ${guesthouseId}`}`;
        XLSX.utils.book_append_sheet(workbook, messSheet, sheetName);
      }

      // Create sheets for each selected city hall (Gedung Nasional)
      this.selectedCityHalls.forEach((cityHallId, index) => {
        const cityHall = this.cityHallsList.find(c => c.id === cityHallId);
        const cityHallTransactions = nationalTransactions.filter(rental => 
          (rental.payment?.status === 'paid' || rental.payment?.status === 'dibayar') &&
          rental.city_hall_pricing?.activity_type &&
          rental.city_hall_pricing?.city_hall?.id === cityHallId
        );
        ...
        const nationalData = cityHallTransactions.map((rental, dataIndex) => [
          dataIndex + 1,
          this.formatDateExcel(rental.created_at),
          rental.renter?.fullname || '-',
          this.formatDateExcel(rental.start_date),
        ...
        // Calculate total for this city hall
        ...
        // Add "Tidak ada data" row if no transactions
        if (nationalData.length === 0) {
          nationalSheetData.push(['', '', 'Tidak ada transaksi dalam periode ini', '', '', '', '', '']);
        }
        ...
        // Use index-based naming to avoid long sheet names
        const sheetName = `Transaksi Gedung Nasional ${index + 1}`;
        XLSX.utils.book_append_sheet(workbook, nationalSheet, sheetName);
      });
      ...
      // Add mess summary - filter paid transactions for each selected guesthouse
      this.selectedGuesthouses.forEach(guesthouseId => {
        const guesthouse = this.guesthousesList.find(g => g.id === guesthouseId);
        const transactions = messTransactions.filter(rental => 
          (rental.payment?.status === 'paid' || rental.payment?.status === 'dibayar') &&
          rental.guesthouse_room_pricing?.retribution_type &&
          rental.guesthouse_room_pricing?.guesthouse_room?.guesthouse?.id === guesthouseId
        );
      ...
      // Add national building summary - filter paid transactions for each selected city hall
      this.selectedCityHalls.forEach(cityHallId => {
        const cityHall = this.cityHallsList.find(c => c.id === cityHallId);
        const transactions = nationalTransactions.filter(rental => 
          (rental.payment?.status === 'paid' || rental.payment?.status === 'dibayar') &&
          rental.city_hall_pricing?.activity_type &&
          rental.city_hall_pricing?.city_hall?.id === cityHallId
        );
      ...
      // Add "Tidak ada data" if no summary data
      if (summaryData.length === 0) {
        summarySheetData.push(['', 'Tidak ada transaksi dalam periode ini', '', '']);
      }
      ...
      // Generate Excel file with building names in filename for better identification
      const selectedBuildingNames = [
        ...this.selectedGuesthouses.map(id => {
          const guesthouse = this.guesthousesList.find(g => g.id === id);
          return guesthouse?.name || `Mess_${id}`;
        }),
        ...this.selectedCityHalls.map(id => {
          const cityHall = this.cityHallsList.find(c => c.id === id);
          return cityHall?.name || `Gedung_${id}`;
        })
      ];
      
      const buildingPart = selectedBuildingNames.length > 3 
        ? `${selectedBuildingNames.slice(0, 2).join('_')}_dan_lainnya`
        : selectedBuildingNames.join('_');
      
      const fileName = `Laporan_Keuangan_${buildingPart}_${monthName}_${year}.xlsx`;
