    ...
    <div class="container mt-6 mx-auto px-6 py-8">
      ...
      <div class="mt-8 bg-blue-100 p-6 rounded-xl shadow-xl">
        ...
        <div v-if="activeFilters().length > 0" class="mt-4 mb-4">
          <div class="flex intems-center flex-wrap gap-2">
            <div 
              v-for="filter in activeFilters()" 
              :key="filter.key"
              class="inline-flex items-center bg-blue-500 text-white px-3 py-1 rounded-full text-sm font-medium"
            >
              <span>{{ filter.label }}</span>
              <button 
                @click="removeFilter(filter.key)"
                class="ml-2 text-white hover:text-white font-bold text-lg leading-none"
                title="Hapus filter"
              >
                ×
              </button>
...
export default {
  ...
  methods: {
    ...
    activeFilters() {
      ...
      // Date range filter
      if (!this.filterAllDates && this.filterStartDate && this.filterEndDate) {
        const startDate = this.formatDateIndicator(this.filterStartDate);
        const endDate = this.formatDateIndicator(this.filterEndDate);
        filters.push({
          key: 'dateRange',
      ...
      if (this.selectedPaymentStatuses.length > 0) {
        const statusLabels = this.selectedPaymentStatuses.map(status => {
