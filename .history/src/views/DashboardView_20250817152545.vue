...
export default {
  ...
  methods: {
    ...
    // Handle page click (normal page or ellipsis)
    handlePageClick(page) {
      if (page === '...') {
        this.showPageInputDialog();
      } else {
        this.goToPage(page);
      }
    },

    // Get CSS class for page button
    getPageButtonClass(page) {
      const baseClass = 'pagination-btn';
      const classes = [baseClass];
      
      if (page === '...') {
        classes.push('pagination-ellipsis');
      }
    ...
    // Get title attribute for page button
    getPageButtonTitle(page) {
      if (page === '...') {
        return 'Masukkan nomor halaman';
      }
      return `Halaman ${page}`;
    },
