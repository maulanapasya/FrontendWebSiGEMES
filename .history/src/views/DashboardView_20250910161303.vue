...
export default {
  ...
  try {
    ...
    // Menyimpan semua data transaksi gedung yang dipilih
    const allRentals = [];

    // Fetch data untuk setiap guesthouse yang dipilih
    for (const guesthouseId of this.selectedGuesthouses) {
      const rentals = await this.fetchRentalsByBuilding('guesthouse', guesthouseId, startDateStr, endDateStr);
      allRentals.push(...rentals); // Gabungkan semua transaksi guesthouse
    }

    // Fetch data untuk setiap city hall yang dipilih
    for (const cityHallId of this.selectedCityHalls) {
      const rentals = await this.fetchRentalsByBuilding('city_hall', cityHallId, startDateStr, endDateStr);
      allRentals.push(...rentals); // Gabungkan semua transaksi city hall
    }

    // Ekspor file terpisah berdasarkan gedung
    const uniqueBuildings = [
      ...new Set([
        ...this.selectedGuesthouses.map((id) => `Guesthouse ${id}`),
        ...this.selectedCityHalls.map((id) => `City Hall ${id}`),
      ])
    ];

    // Ekspor file untuk masing-masing gedung yang dipilih
    for (const building of uniqueBuildings) {
      const buildingRentals = allRentals.filter((rental) => {
        if (building.includes('Guesthouse')) {
          return rental.guesthouse_room_pricing?.guesthouse?.name === building;
        } else if (building.includes('City Hall')) {
          return rental.city_hall_pricing?.city_hall?.name === building;
        }
      });

      const buildingName = building.replace('Guesthouse', 'Mess').replace('City Hall', 'Gedung Nasional');
      const fileName = `Laporan_Keuangan_${buildingName}_${month}_${year}.xlsx`;

      const workbook = XLSX.utils.book_new();
      const sheetData = this.prepareExcelData(buildingRentals);
      const sheet = XLSX.utils.aoa_to_sheet(sheetData);

      // Tambahkan sheet ke workbook
      XLSX.utils.book_append_sheet(workbook, sheet, `Transaksi ${buildingName}`);

      // Simpan file
      XLSX.writeFile(workbook, fileName);
    }

    // Ekspor file ringkasan keuangan
    const summaryFileName = `Ringkasan_Keuangan_${month}_${year}.xlsx`;
    const summaryData = this.prepareSummaryData(this.selectedCityHalls, this.selectedGuesthouses, startDateStr, endDateStr);
    const summaryWorkbook = XLSX.utils.book_new();
    const summarySheet = XLSX.utils.aoa_to_sheet(summaryData);

    // Tambahkan data ringkasan ke workbook
    XLSX.utils.book_append_sheet(summaryWorkbook, summarySheet, 'Ringkasan Keuangan');

    // Simpan file ringkasan
    ...
    // Prepare the data for Excel (transactions for each building)
    prepareExcelData(rentals) {
      const header = ['No', 'Tanggal Transaksi', 'Nama Penyewa', 'Tanggal Check-in', 'Tanggal Check-out', 'Nomor Invoice', 'Jenis Retribusi', 'Harga Item (@)', 'Total Harga'];
      const rows = rentals.map((rental, index) => [
        ...
        rental.payment?.id || '-',
        rental.guesthouse_room_pricing?.retribution_type || rental.city_hall_pricing?.activity_type || '-',
        rental.guesthouse_room_pricing?.price_per_day || rental.city_hall_pricing?.price_per_day || 0,
        rental.payment?.amount || 0
      ]);

      rows.push(['', '', '', '', '', '', 'Jumlah', '', rows.reduce((sum, row) => sum + (row[8] || 0), 0)]);

      return [header, ...rows];
    },

    // Prepare summary data for the financial summary
    prepareSummaryData(cityHalls, guesthouses, startDateStr, endDateStr) {
      const summary = [];
      let totalRenters = 0;
      let totalRevenue = 0;

      // Process each selected guesthouse
      guesthouses.forEach(guesthouseId => {
        const guesthouse = this.guesthousesList.find(g => g.id === guesthouseId);
        const rentals = this.fetchRentalsByBuilding('guesthouse', guesthouseId, startDateStr, endDateStr);
        const revenue = rentals.reduce((sum, rental) => sum + (rental.payment?.amount || 0), 0);

        summary.push(['Mess', guesthouse.name, rentals.length, revenue]);
        totalRenters += rentals.length;
        totalRevenue += revenue;
      });

      // Process each selected city hall
      cityHalls.forEach(cityHallId => {
        const cityHall = this.cityHallsList.find(c => c.id === cityHallId);
        const rentals = this.fetchRentalsByBuilding('city_hall', cityHallId, startDateStr, endDateStr);
        const revenue = rentals.reduce((sum, rental) => sum + (rental.payment?.amount || 0), 0);

        summary.push(['Gedung Nasional', cityHall.name, rentals.length, revenue]);
        totalRenters += rentals.length;
        totalRevenue += revenue;
      });

      // Add total row
      summary.push(['', 'TOTAL', totalRenters, totalRevenue]);

      return [['Jenis Gedung', 'Nama Gedung', 'Jumlah Penyewa', 'Total Pendapatan'], ...summary];
    },
    ...
    async fetchRentalsByBuilding(type, buildingId, startDate, endDate) {
      ...
      try {
        ...
        // Sesuaikan endpoint berdasarkan tipe gedung (guesthouse atau city_hall)
        let endpoint = '';
        if (type === 'guesthouse') {
          endpoint = `https://sigemes-api-63593591732.us-central1.run.app/api/v1/rents?guesthouse_id=${buildingId}&${params}`;
        } else if (type === 'city_hall') {
          endpoint = `https://sigemes-api-63593591732.us-central1.run.app/api/v1/rents?city_hall_id=${buildingId}&${params}`;
        }
        ...
        if (response.data.status) {
          rentals = response.data.data; // Simpan data transaksi
        }
      } catch (error) {
        console.error('Error fetching rentals:', error);
      }

      return rentals; // Pastikan mengembalikan array yang valid
    },
