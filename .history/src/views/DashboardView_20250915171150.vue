...
export default {
  ...
  methods: {
    ...
    async addLogo(worksheet, startRow = 1, startCol = 1) {
      try {
        ...
        if (response.ok) {
          ...
          // PERBAIKAN: Logo lebih simetris dan proporsional
          worksheet.addImage(imageId, {
            tl: { col: startCol - 1, row: startRow - 1 },
            ext: { width: 70, height: 85 }, // Ukuran lebih proporsional
    ...
    async createBuildingReport(transactions, buildingName, buildingType, cleanBuildingName, monthName, year) {
      ...
      // TAMBAH LOGO - FITUR BARU
      await this.addLogo(worksheet, 1, 1);

      // Set column widths
      if (buildingType === 'Mess') {
        worksheet.columns = [
          { width: 5 }, { width: 20 }, { width: 25 }, { width: 25 }, 
          { width: 25 }, { width: 45 }, { width: 35 }, { width: 15 }, { width: 18 }
        ];
      } else {
        worksheet.columns = [
          { width: 5 }, { width: 20 }, { width: 25 }, { width: 20 }, 
          { width: 40 }, { width: 20 }, { width: 15 }, { width: 18 }
        ];
      }
      ...
      // Apply header dengan styling - FITUR BARU
      for (let i = 0; i < headerData.length; i++) {
        ...
        this.applyCellStyleExcelJS(worksheet, `B${row}`, {
          font: { 
            size: i === 1 ? 18 : 14, // Sekretariat Daerah = 18
            bold: i === 0 || i === 1
          },
      ...
      // Border bawah kop surat - FITUR BARU
      const lastCol = buildingType === 'Mess' ? 'I' : 'H';
      ...
      // Month info - DIPERBAIKI WIDTH
      const monthRow = 11;
      worksheet.getCell(`A${monthRow}`).value = `Bulan : ${monthName} ${year}`;
      worksheet.mergeCells(`A${monthRow}:D${monthRow}`); // Merge lebih banyak kolom
      this.applyCellStyleExcelJS(worksheet, `A${monthRow}`, {
        font: { size: 12 },
        alignment: { horizontal: 'left', vertical: 'middle' }
      });

      // Table header dengan styling
      const headerRow = 13;
      let tableHeaders;
      
      if (buildingType === 'Mess') {
        tableHeaders = ['No', 'Tanggal Transaksi', 'Nama Penyewa', 'Tanggal Check-in', 'Tanggal Check-out', 'Nomor Invoice', 'Jenis Retribusi', 'Harga Item (@)', 'Total Harga'];
      } else {
        tableHeaders = ['No', 'Tanggal Transaksi', 'Nama Penyewa', 'Tanggal Pemakaian', 'Nomor Invoice', 'Jenis Retribusi', 'Harga Item (@)', 'Total Harga'];
      }
      ...
      // Total row dengan styling
      const totalRow = currentRow;
      const total = paidTransactions.reduce((sum, rental) => sum + (rental.payment?.amount || 0), 0);
      
      const jumlahCol = tableHeaders.length - 2;
      const totalCol = tableHeaders.length - 1;
      
      worksheet.getCell(String.fromCharCode(65 + jumlahCol) + totalRow).value = 'Jumlah';
      worksheet.getCell(String.fromCharCode(65 + totalCol) + totalRow).value = total;
      ...
      for (let colIndex = 0; colIndex < tableHeaders.length; colIndex++) {
        ...
        this.applyCellStyleExcelJS(worksheet, `${colLetter}${totalRow}`, {
          ...
          alignment: colIndex === jumlahCol || colIndex === totalCol 
            ? { horizontal: 'right', vertical: 'middle' }
            : { horizontal: 'center', vertical: 'middle' }
        });
        
        if (colIndex === totalCol) {
          worksheet.getCell(`${colLetter}${totalRow}`).numFmt = '#,##0';
        }
      }
    ...
    async createSummaryReport(allBuildingData, monthName, year) {
      ...
      // Set column widths
      worksheet.columns = [
        { width: 5 }, { width: 40 }, { width: 15 }, { width: 20 }
      ];
      ...
      const grandTotalPendapatan = allBuildingData.reduce((sum, building) => {
        return sum + building.transactions.reduce((buildingSum, t) => buildingSum + (t.payment?.amount || 0), 0);
      }, 0);
      ...
      // SIGNATURE SECTION - FITUR BARU sesuai screenshot 3
      const signatureStartRow = totalRow + 3;
      
      worksheet.getCell(`D${signatureStartRow}`).value = 'Padangsidimpuan, [hari bulan tahun]';
      ...
      // Names (4 rows space untuk tanda tangan)
      const nameRow = signatureRow4 + 5;
      ...
      for (let row = signatureStartRow; row <= nipRow; row++) {
        ['A', 'D'].forEach(col => {
          this.applyCellStyleExcelJS(worksheet, `${col}${row}`, {
            font: { size: 12 },
            alignment: { horizontal: 'center', vertical: 'middle' }
          });
        });
