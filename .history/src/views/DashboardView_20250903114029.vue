  ...
  <div>
    ...
    <transition name="modal">
      <div v-if="showFilterModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click="closeFilterModal">
        <div class="bg-blue-100 rounded-lg max-w-lg w-full p-6" @click.stop>
          <h2 class="rent-column text-2xl font-bold text-black text-center mb-6">Filter Berdasarkan</h2>
          ...
          <div class="mb-4">
            ...
            <!-- Date Range Picker - Vue Datepicker Only -->
            <div v-if="!filterAllDates" class="mb-3">
              <VueDatePicker
                v-model="dateRange"
                range
            ...
            <!-- Date Range Inputs - COMMENTED OUT as requested -->
            <!-- 
            <div v-if="!filterAllDates && !useVueDatepicker" class="flex items-center gap-2 bg-white p-3 rounded-lg border border-gray-300">
              ...
              <input 
                ...
                type="date"
                class="flex-1 bg-transparent text-black focus:outline-none"
            ...
            <!-- Toggle Calendar Type Button - COMMENTED OUT since only using Vue DatePicker -->
            <!-- 
            <div class="mt-2" v-if="!filterAllDates">
              <button 
                @click="toggleDatepickerType"
                type="button"
                class="text-sm text-blue-500 hover:text-blue-700 font-medium">
                {{ useVueDatepicker ? '📅 Gunakan Input Tanggal Biasa' : '📅 Gunakan Calendar Picker' }}
              </button>
            </div>
            -->
          </div>

          <!-- Payment Status - Custom Dropdown with Auto Width -->
          <div class="mb-4">
            <h3 class="rent-column font-bold text-black mb-3">Status Pembayaran</h3>
            <div class="relative">
              <div class="flex items-center gap-2 bg-white border border-gray-300 rounded-lg px-3 py-2.5 cursor-pointer hover:border-blue-400 transition-colors w-fit min-w-[140px]"
                  @click="togglePaymentStatusDropdown">
                <span class="text-gray-600 text-sm flex-1">{{ getPaymentStatusLabel() }}</span>
                <svg class="w-4 h-4 text-gray-500 transition-transform ml-2" :class="{ 'rotate-180': showPaymentStatusDropdown }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
              </div>
              
              <!-- Payment Status Dropdown Menu -->
              <div v-if="showPaymentStatusDropdown" 
                  class="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 min-w-[140px] w-auto">
                <div class="py-1">
                  <div v-for="option in paymentStatusOptions" 
                      :key="option.value"
                      @click="selectPaymentStatus(option.value)"
                      class="px-3 py-2 text-sm text-black hover:bg-blue-50 cursor-pointer whitespace-nowrap"
                      :class="{ 'bg-blue-100 font-medium': filterPaymentStatus === option.value }">
                    {{ option.label }}
                  </div>
                </div>
              </div>
          ...
          <!-- Rental Type - Custom Dropdown with Auto Width -->
          <div class="mb-4">
            <h3 class="rent-column font-bold text-black mb-3">Jenis Sewa</h3>
            <div class="relative">
              <div class="flex items-center gap-2 bg-white border border-gray-300 rounded-lg px-3 py-2.5 cursor-pointer hover:border-blue-400 transition-colors w-fit min-w-[160px]"
                  @click="toggleRentalTypeDropdown">
                <span class="text-gray-600 text-sm flex-1">{{ getRentalTypeLabel() }}</span>
                <svg class="w-4 h-4 text-gray-500 transition-transform ml-2" :class="{ 'rotate-180': showRentalTypeDropdown }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
              </div>
              ...
              <div v-if="showRentalTypeDropdown" 
                  class="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 min-w-[160px] w-auto">
                <div class="py-1">
          ...
          <template v-if="filterRentalType === 'guesthouse'">
            <!-- Room Type - Custom Dropdown with Auto Width -->
            <div class="mb-4">
              <h3 class="rent-column font-bold text-black mb-3">Tipe</h3>
              <div class="relative">
                <div class="flex items-center gap-2 bg-white border border-gray-300 rounded-lg px-3 py-2.5 cursor-pointer hover:border-blue-400 transition-colors w-fit min-w-[100px]"
                    @click="toggleRoomTypeDropdown">
                  <span class="text-gray-600 text-sm flex-1">{{ getRoomTypeLabel() }}</span>
                  <svg class="w-4 h-4 text-gray-500 transition-transform ml-2" :class="{ 'rotate-180': showRoomTypeDropdown }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                  </svg>
                </div>
                ...
                <div v-if="showRoomTypeDropdown" 
                    class="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 min-w-[100px] w-auto">
                  <div class="py-1">
            ...
            <!-- Retribution - Custom Dropdown with Auto Width -->
            <div class="mb-4">
              <h3 class="rent-column font-bold text-black mb-3">Retribusi</h3>
              <div class="relative">
                <div class="flex items-center gap-2 bg-white border border-gray-300 rounded-lg px-3 py-2.5 cursor-pointer hover:border-blue-400 transition-colors w-fit min-w-[200px] max-w-[400px]"
                    @click="toggleRetributionDropdown">
                  <span class="text-gray-600 text-sm flex-1 truncate">{{ getRetributionLabel() }}</span>
                  <svg class="w-4 h-4 text-gray-500 transition-transform ml-2 flex-shrink-0" :class="{ 'rotate-180': showRetributionDropdown }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                  </svg>
                </div>
                ...
                <div v-if="showRetributionDropdown" 
                    class="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 min-w-[200px] max-w-[400px] w-auto">
                  <div class="py-1">
          ...
          <template v-if="filterRentalType === 'city_hall'">
            <div class="mb-4">
              ...
              <div class="relative">
                <div class="flex items-center gap-2 bg-white border border-gray-300 rounded-lg px-3 py-2.5 cursor-pointer hover:border-blue-400 transition-colors w-fit min-w-[180px]"
                    @click="toggleActivityTypeDropdown">
                  <span class="text-gray-600 text-sm flex-1">{{ getActivityTypeLabel() }}</span>
                  <svg class="w-4 h-4 text-gray-500 transition-transform ml-2" :class="{ 'rotate-180': showActivityTypeDropdown }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                  </svg>
                </div>
                ...
                <div v-if="showActivityTypeDropdown" 
                    class="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-10 min-w-[180px] w-auto">
                  <div class="py-1">
          ...
          <!-- Disabled Field for "Semua" -->
          <div v-if="!filterRentalType" class="mb-4">
            <h3 class="rent-column font-bold text-gray-400 mb-3">-</h3>
            <div class="flex items-center gap-2 bg-gray-100 border border-gray-200 rounded-lg px-3 py-2.5 cursor-not-allowed w-fit min-w-[60px]">
              <span class="text-gray-400 text-sm flex-1">-</span>
...
export default {
  ...
  data() {
    return {
      ...
      // Date picker options
      useVueDatepicker: false, // Toggle between Vue Datepicker and native inputs
  ...
  methods: {
    ...
    // Update closeFilterModal to close dropdowns
    closeFilterModal() {
    ...
    handleAllDatesToggle() {
      if (this.filterAllDates) {
        ...
        // Set default to today when unchecked
        const today = new Date().toISOString().split('T')[0];
        this.filterStartDate = today;
        this.filterEndDate = today;
      }
    ...
    async fetchRentals() {
      ...
      try {
        ...
        if (this.filterPaymentStatus) {
          params.append('payment_status', this.filterPaymentStatus);
        }

        // Add rental type filter
        if (this.filterRentalType) {
          if (this.filterRentalType === 'guesthouse') {
            params.append('guesthouse_only', 'true');
          } else if (this.filterRentalType === 'city_hall') {
            params.append('city_hall_only', 'true');
          }
        }

        // Add room type filter for guesthouse
        if (this.filterRentalType === 'guesthouse' && this.filterRoomType) {
          params.append('room_type', this.filterRoomType);
        }

        // Add retribution filter for guesthouse
        if (this.filterRentalType === 'guesthouse' && this.filterRetribution) {
          params.append('retribution_type', this.filterRetribution);
        }

        // Add activity type filter for city hall
        if (this.filterRentalType === 'city_hall' && this.filterActivityType) {
          params.append('activity_type', this.filterActivityType);
        }
