      ...
      <div class="flex flex-wrap gap-4 mt-6">
        ...
        <div class="bg-blue-100 p-6 rounded-xl shadow-lg flex-1">
          <p class="text-gray-800">
            ...
            <span v-else>Total pendapatan tahun <span class="font-">{{ currentYear }}</span></span>
          </p>
          <h2 class="revenue text-3xl text-black font-bold">
            <span v-if="loadingRevenue" class="skeleton-loader inline-block w-48 h-9 rounded"></span>
    ...
    <transition name="modal">
      <div v-if="showExportModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click="closeExportModal">
        <div class="bg-white rounded-lg max-w-2xl w-full p-6" @click.stop>
          <!-- Modal Header -->
          <h2 class="text-2xl font-bold text-black text-center mb-6">Ekspor Laporan Keuangan</h2>
  ...
  methods: {
    ...
    async createExcelReport(messTransactions, nationalTransactions, month, year) {
      ...
      const messData = paidMessTransactions.map((rental, index) => ({
        'No': index + 1,
        'Tanggal Transaksi': this.formatDateExcel(rental.created_at),
        'Nama Penyewa': rental.renter?.fullname || '-',
        'Tanggal Check-in': this.formatDateExcel(rental.start_date),
        'Tanggal Check-out': this.formatDateExcel(rental.end_date),
        'Nomor Invoice': rental.payment?.id || '-',
        'Jenis Retribusi': rental.guesthouse_room_pricing?.retribution_type || '-',
        'Harga Item (@)': rental.guesthouse_room_pricing?.price_per_day || 0,
        'Total Harga': rental.payment?.amount || 0
      }));

      // Calculate total for mess
      const messTotal = messData.reduce((sum, row) => sum + (row['Total Harga'] || 0), 0);
      ...
      messData.push({
        ...
        'Tanggal Check-out': '',
        'Nomor Invoice': '',
        'Jenis Retribusi': 'Jumlah',
        'Harga Item (@)': '',
        'Total Harga': messTotal
      });

      const messSheet = XLSX.utils.json_to_sheet(messData);
      
      // Apply styling to mess sheet
      this.applySheetStyling(messSheet, messData.length, 9);
      ...
      const nationalData = paidNationalTransactions.map((rental, index) => ({
        ...
        'Tanggal Transaksi': this.formatDateExcel(rental.created_at),
        'Nama Penyewa': rental.renter?.fullname || '-',
        'Tanggal Pemakaian': this.formatDateExcel(rental.start_date),
        'Nomor Invoice': rental.payment?.id || '-',
        'Jenis Retribusi': rental.city_hall_pricing?.activity_type || '-',
        'Harga Item (@)': rental.city_hall_pricing?.price_per_day || 0,
        'Total Harga': rental.payment?.amount || 0
      }));

      // Calculate total for national
      const nationalTotal = nationalData.reduce((sum, row) => sum + (row['Total Harga'] || 0), 0);
      ...
      nationalData.push({
        ...
        'Jenis Retribusi': 'Jumlah',
        'Harga Item (@)': '',
        'Total Harga': nationalTotal
      });

      const nationalSheet = XLSX.utils.json_to_sheet(nationalData);
      
      // Apply styling to national sheet
      this.applySheetStyling(nationalSheet, nationalData.length, 8);
      ...
      this.selectedGuesthouses.forEach(guesthouseId => {
        ...
        const transactions = paidMessTransactions.filter(t => t.guesthouse_id === guesthouseId);
        const totalAmount = transactions.reduce((sum, t) => sum + (t.payment?.amount || 0), 0);
        
        summaryData.push({
          'Jenis Gedung': 'Mess',
          'Nama Gedung': guesthouse?.name || '-',
          'Jumlah Penyewa': transactions.length,
          'Total Pendapatan': totalAmount
        });
      });

      // Add national building summary
      this.selectedCityHalls.forEach(cityHallId => {
        const cityHall = this.cityHallsList.find(c => c.id === cityHallId);
        const transactions = paidNationalTransactions.filter(t => t.city_hall_id === cityHallId);
        const totalAmount = transactions.reduce((sum, t) => sum + (t.payment?.amount || 0), 0);
        
        summaryData.push({
          'Jenis Gedung': 'Gedung Nasional',
          'Nama Gedung': cityHall?.name || '-',
          'Jumlah Penyewa': transactions.length,
          'Total Pendapatan': totalAmount
        });
      });
      ...
      const grandTotalPenyewa = summaryData.reduce((sum, row) => sum + (row['Jumlah Penyewa'] || 0), 0);
      const grandTotalPendapatan = summaryData.reduce((sum, row) => sum + (row['Total Pendapatan'] || 0), 0);

      // Add grand total
      summaryData.push({
        'Jenis Gedung': '',
        'Nama Gedung': 'TOTAL',
        'Jumlah Penyewa': grandTotalPenyewa,
      ...
      this.applySheetStyling(summarySheet, summaryData.length, 4);
    ...
    applySheetStyling(worksheet, rowCount, colCount) {
      ...
      for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        columnWidths.push({ wch: Math.min(maxWidth, 50) }); // max width 50
      }
      ...
      for (let R = range.s.r; R <= range.e.r; ++R) {
        for (let C = range.s.c; C <= range.e.c; ++C) {
          ...
          // Bold for header row (first row) and total row (last row)
          if (R === range.s.r || R === range.e.r) {
            worksheet[cellAddress].s.font = { bold: true };
          }
          
          // Center align for "No" column and numeric columns
          if (C === 0 || (worksheet[cellAddress].t === 'n')) {
