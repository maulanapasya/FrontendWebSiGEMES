...
export default {
  ...
  methods: {
    ...
    // Create building report method
    async createBuildingReport(transactions, buildingName, buildingType, cleanBuildingName, monthName, year) {
      try {
        // console.log(`Starting createBuildingReport for: ${buildingName} (${buildingType})`);
        // console.log(`Input data - transactions: ${transactions.length}, cleanBuildingName: ${cleanBuildingName}`);
        ...
        // console.log(`Paid transactions: ${paidTransactions.length} out of ${transactions.length}`);
        ...
        const headerData = [
          ...
          [''],
          [''],
          [`Laporan Transaksi ${buildingName} - ${monthName} ${year}`],
          ['']
        ];
        ...
        if (buildingType === 'Mess') {
          ...
          // console.log('Processing city hall data...');
          ...
          tableData = paidTransactions.map((rental, index) => {
            ...
            //   id: rental.id,
            //   renter: rental.renter?.fullname,
            //   payment: rental.payment?.id,
            //   activity_type: rental.city_hall_pricing?.activity_type,
        ...
        // console.log(`Table data created with ${tableData.length} rows`);
        ...
        totalRow[tableData[0].length - 3] = 'Jumlah'; // "Jumlah" in the appropriate column
        totalRow[tableData[0].length - 1] = total; // Total in the last column
        ...
        // console.log(`Creating worksheet with ${sheetData.length} total rows`);
        ...
        const headerMerges = [
          ...
          { s: { r: 2, c: 0 }, e: { r: 2, c: tableHeader.length - 1 } }, // Jalan
          { s: { r: 3, c: 0 }, e: { r: 3, c: tableHeader.length - 1 } }, // Telepon
          { s: { r: 4, c: 0 }, e: { r: 4, c: tableHeader.length - 1 } }, // Faksimile
          { s: { r: 7, c: 0 }, e: { r: 7, c: tableHeader.length - 1 } }  // Laporan title
        ];
        ...
        // Generate Excel file with enhanced download method for Safari compatibility
        const fileName = `Laporan_Keuangan_${cleanBuildingName}_${monthName}_${year}.xlsx`;
        // console.log(`Attempting to write file: ${fileName}`);
        
        try {
          // Try different write methods for better browser compatibility
          const wbout = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
          const blob = new Blob([wbout], { type: 'application/octet-stream' });
          
          // Create download link manually (better Safari support)
          ...
          // console.log(`✓ Successfully created and forced download: ${fileName}`);
        } catch (downloadError) {
          console.error('Error in manual download:', downloadError);
        ...
        console.error(`ERROR in createBuildingReport for ${buildingName}:`, error);
    ...
    // UPDATED FETCH RENTALS BY BUILDING WITH ENHANCED DEBUGGING
    async fetchRentalsByBuilding(type, buildingId, startDate, endDate) {
      ...
      console.log(`Fetching rentals for ${type} with ID ${buildingId} from ${startDate} to ${endDate}`);
      ...
      while (hasMore) {
        try {
          ...
          if (response.data.status && response.data.data.length > 0) {
            ...
            // Debug: Log all rentals to understand the structure
            console.log(`Checking ${response.data.data.length} rentals for filtering...`);
            
            for (let i = 0; i < Math.min(3, response.data.data.length); i++) {
              const rental = response.data.data[i];
              console.log(`Sample rental ${i + 1} structure:`, {
                id: rental.id,
                guesthouse_room_pricing: rental.guesthouse_room_pricing ? {
                  id: rental.guesthouse_room_pricing.id,
                  guesthouse_room: rental.guesthouse_room_pricing.guesthouse_room ? {
                    guesthouse: rental.guesthouse_room_pricing.guesthouse_room.guesthouse ? {
                      id: rental.guesthouse_room_pricing.guesthouse_room.guesthouse.id,
                      name: rental.guesthouse_room_pricing.guesthouse_room.guesthouse.name
                    } : null
                  } : null
                } : null,
                city_hall_pricing: rental.city_hall_pricing ? {
                  id: rental.city_hall_pricing.id,
                  city_hall: rental.city_hall_pricing.city_hall ? {
                    id: rental.city_hall_pricing.city_hall.id,
                    name: rental.city_hall_pricing.city_hall.name
                  } : null
                } : null
              });
            }

            // Filter by building type and ID on client side (debug only)
            // filteredRentals = response.data.data.filter(rental => {
            //   if (type === 'guesthouse') {
            //     const guesthouseId = rental.guesthouse_room_pricing?.guesthouse_room?.guesthouse?.id;
            //     const match = guesthouseId === buildingId;
            //     if (match) {
            //       console.log(`✓ Guesthouse match found: rental ID ${rental.id}, guesthouse ID ${guesthouseId}`);
            //     }
            //     return match;
            //   } else if (type === 'city_hall') {
            //     const cityHallId = rental.city_hall_pricing?.city_hall?.id;
            //     const match = cityHallId === buildingId;
            //     console.log(`City hall check: rental ID ${rental.id}, expected ID ${buildingId}, actual ID ${cityHallId}, match: ${match}`);
            //     return match;
            //   }
            //   return false;
            // });

    //         console.log(`Filtered ${filteredRentals.length} rentals for ${type} ID ${buildingId}`);
    ...
    //             } catch (error) {
    //               console.error(`Error fetching renter ${rental.renter_id}:`, error);
    //               rental.renter = { fullname: 'Error loading name' };
    //             }
    //           }
    //         }
    ...
    //       }
    //     } catch (error) {
    //       console.error(`Error fetching rentals page ${page}:`, error);
    //       hasMore = false;
    //     }
    //   }

    //   console.log(`Final result: ${rentals.length} rentals found for ${type} ID ${buildingId}`);
    //   return rentals;
    // },
