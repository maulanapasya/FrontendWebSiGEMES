        ...
        <div v-if="activeFilters().length > 0" class="mt-4 mb-4">
          <div class="flex flex-wrap gap-2">
            <div 
              ...
              :key="filter.key"
              class="inline-flex items-center bg-gray-200 text-gray-800 px-3 py-1 rounded-full text-sm font-medium"
            >
              <span>{{ filter.label }}</span>
              <button 
                @click="removeFilter(filter.key)"
                class="ml-2 text-gray-600 hover:text-gray-800 font-bold text-lg leading-none"
                title="Hapus filter"
              >
                ×
              </button>
