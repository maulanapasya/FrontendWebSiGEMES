      ...
      <div class="flex flex-wrap gap-4 mt-6">
        <div class="bg-blue-100 p-6 rounded-xl shadow-lg flex-1">
          ...
          <h2 class="revenue text-3xl text-black">
            ...
            <span v-else class="font-black"><span class="font-bold">{{ formatCurrency(dailyRevenue) }}</span></span>
          </h2>
        </div>
        <div class="bg-blue-100 p-6 rounded-xl shadow-lg flex-1">
          <p class="text-gray-800">
...
export default {
  ...
  methods: {
    ...
    async createExcelReport(messTransactions, nationalTransactions, month, year) {
      ...
      const messData = messTransactions.map((rental, index) => ({
        'No': index + 1,
      ...
      messData.push({
        ...
        'Harga Item (@)': '',
        'Total Harga': messData.reduce((sum, row) => sum + (row['Total Harga'] || 0), 0)
      });
      ...
      const nationalData = nationalTransactions.map((rental, index) => ({
        'No': index + 1,
        'Tanggal Transaksi': this.formatDateExcel(rental.created_at),
        'Nama Penyewa': rental.renter?.fullname || '-',
      ...
      nationalData.push({
        ...
        'Jenis Retribusi': 'Jumlah',
        'Harga Item (@)': '',
        'Total Harga': nationalData.reduce((sum, row) => sum + (row['Total Harga'] || 0), 0)
      });
      ...
      this.selectedGuesthouses.forEach(guesthouseId => {
        ...
        const transactions = messTransactions.filter(t => t.guesthouse_id === guesthouseId);
        summaryData.push({
          'Jenis Gedung': 'Mess',
          'Nama Gedung': guesthouse?.name || '-',
          'Jumlah Penyewa': transactions.length,
          'Total Pendapatan': transactions.reduce((sum, t) => sum + (t.payment?.amount || 0), 0)
        });
      });
      ...
      this.selectedCityHalls.forEach(cityHallId => {
        ...
        const transactions = nationalTransactions.filter(t => t.city_hall_id === cityHallId);
        summaryData.push({
      ...
      summaryData.push({
        'Jenis Gedung': '',
        'Nama Gedung': 'TOTAL',
        'Jumlah Penyewa': summaryData.reduce((sum, row) => sum + (row['Jumlah Penyewa'] || 0), 0),
        'Total Pendapatan': summaryData.reduce((sum, row) => sum + (row['Total Pendapatan'] || 0), 0)
      });
