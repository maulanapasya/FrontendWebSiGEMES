  ...
  methods: {
    ...
    async exportToExcel() {
      ...
      try {
        ...
        // Initialize an array to store the promises for fetching rental data
        const rentalPromises = [];

        // Fetch data for each selected guesthouse
        for (const guesthouseId of this.selectedGuesthouses) {
          rentalPromises.push(this.fetchRentalsByBuilding('guesthouse', guesthouseId, startDateStr, endDateStr));
        }

        // Fetch data for each selected city hall
        for (const cityHallId of this.selectedCityHalls) {
          rentalPromises.push(this.fetchRentalsByBuilding('city_hall', cityHallId, startDateStr, endDateStr));
        }

        // Wait for all rental data to be fetched
        const allRentals = await Promise.all(rentalPromises);

        // Create Excel files for each building
        for (const rentals of allRentals) {
          const buildingId = rentals[0]?.guesthouse_room_pricing?.guesthouse_id || rentals[0]?.city_hall_pricing?.city_hall_id;
          const buildingName = rentals[0]?.guesthouse_room_pricing?.guesthouse?.name || rentals[0]?.city_hall_pricing?.city_hall?.name;
          const fileName = `Laporan_Keuangan_${buildingName}_${month}_${year}.xlsx`;

          // Create a workbook for each building's transactions
          const workbook = XLSX.utils.book_new();
          const sheetData = this.prepareExcelData(rentals);
        ...
        // Create Ringkasan Keuangan file
        ...
        // Append summary data to workbook
