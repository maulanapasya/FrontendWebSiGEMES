...
export default {
  ...
  methods: {
    ...
    // Updated method to create individual building report
...
async createSummaryReport(allBuildingData, monthName, year) {
  ...
  XLSX.writeFile(workbook, fileName);
    ...
    // Updated method to create summary report
    async createSummaryReport(allBuildingData, monthName, year) {
      const workbook = XLSX.utils.book_new();
      ...
      const headerData = [
        ...
        [''],                                          // Row 6
        [''],                                          // Row 7
        ['Laporan Keuangan'],                          // Row 8
        ['Pendapatan Bagian Umum'],                    // Row 9
        ['Sekretariat Daerah Kota Padangsidimpuan'],   // Row 10
        ['(SiGEMES)'],                                 // Row 11
        [''],                                          // Row 12
        [''],                                          // Row 13
      ];
      ...
      const summaryTableHeader = ['No', 'Retribusi', 'Jumlah Penyewa', 'Total Pendapatan'];
      const summaryData = [];
      ...
      const signatureSection = [
        [''],
        [''],
        [`Padangsidimpuan, [hari bulan tahun]`],
        [''],
        ...
        [''],
        [''],
        ['Tobonsyah Pulungan, S. Sos, M. M.', '', 'Jenni Sartika'],
        ['Pembina Tingkat I / IVb', '', 'Penata Muda / IIa'],
        ['NIP. 197909091998031002', '', 'NIP. 198405202008012002']
      ];
      ...
      for (let row = range.s.r; row <= range.e.r; row++) {
        for (let col = range.s.c; col <= range.e.c; col++) {
          ...
          // Apply Arial font to all cells
          summarySheet[cellRef].s.font = { name: 'Arial' };
          
          // Header formatting (rows 0-4)
          if (row >= 0 && row <= 4) {
            summarySheet[cellRef].s.alignment = { horizontal: 'center', vertical: 'center' };
            
            if (row === 0) {
              summarySheet[cellRef].s.font = { name: 'Arial', sz: 14, bold: true };
            } else if (row === 1) {
              summarySheet[cellRef].s.font = { name: 'Arial', sz: 18, bold: true };
            } else {
              summarySheet[cellRef].s.font = { name: 'Arial', sz: 14 };
            }
          }
          ...
          // Table header (row 13)
          if (row === 13) {
            summarySheet[cellRef].s.font = { name: 'Arial', sz: 12, bold: true };
            summarySheet[cellRef].s.alignment = { horizontal: 'center', vertical: 'center' };
            summarySheet[cellRef].s.fill = { fgColor: { rgb: 'E8F4FD' } };
          ...
          // Table data rows
          if (row > 13 && row < 13 + summaryData.length + 1) {
            summarySheet[cellRef].s.font = { name: 'Arial', sz: 11 };
            summarySheet[cellRef].s.border = {
              top: { style: 'thin', color: { rgb: '000000' } },
              bottom: { style: 'thin', color: { rgb: '000000' } },
              left: { style: 'thin', color: { rgb: '000000' } },
            ...
            if (col === 2 || col === 3) {
              summarySheet[cellRef].s.alignment = { horizontal: 'right', vertical: 'center' };
              if (col === 3) {
                summarySheet[cellRef].s.numFmt = '#,##0';
              }
            }
            
            // Bold total row
            if (row === 13 + summaryData.length) {
              summarySheet[cellRef].s.font = { name: 'Arial', sz: 11, bold: true };
            }
          }
          
          // Signature section formatting
          if (row > 13 + summaryData.length + 1) {
            summarySheet[cellRef].s.font = { name: 'Arial', sz: 11 };
            
            // Center align signature titles
            if (row === 13 + summaryData.length + 4 || // Mengetahui row
                row === 13 + summaryData.length + 13 || // Names row
                row === 13 + summaryData.length + 14 || // Titles row
                row === 13 + summaryData.length + 15) { // NIP row
              summarySheet[cellRef].s.alignment = { horizontal: 'center', vertical: 'center' };
            }
          }
        }
