<template>
  <div class="login-page">
    <div class="login-container">
      ...
      <h1 class="title text-black font-black login-text text-2xl mb-4 mt-2">SiGEMES Admin</h1>
...
export default {
  ...
  try {
    ...
    if (response.data && response.data.status) {
      ...
      // Menyimpan token, email, fullname dan role (superadmin atau admin biasa)
      localStorage.setItem('authToken', token);
      localStorage.setItem('userEmail', email);
      localStorage.setItem('userFullname', fullname);

      // Simpan role berdasarkan email (superadmin jika email sesuai, selain itu admin biasa)
      const role = email === "superadmin@prisma.io" ? "superadmin" : "admin";
