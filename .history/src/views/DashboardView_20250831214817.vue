                ...
                <template v-if="rentalDetail.guesthouse_room_pricing">
                  ...
                  <div class="flex" v-if="getRoomType(rentalDetail)">
                    ...
                    <span class="font-medium text-black flex-1 text-sm flex items-center">
                      <img 
                        v-if="getRoomTypeIcon(getRoomType(rentalDetail))" 
                        :src="getRoomTypeIcon(getRoomType(rentalDetail))" 
                        :alt="formatRoomTypeDisplay(getRoomType(rentalDetail))" 
                        class="h-5 w-5 mr-2" 
                      />
                      <span>{{ formatRoomTypeDisplay(getRoomType(rentalDetail)) }}</span>
                    </span>
                  </div>
                </template>
              ...
              <div v-else class="space-y-2 max-h-40 overflow-y-auto border border-gray-400 rounded-lg p-3">
                <label v-for="hall in cityHallsList" :key="hall.id" class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                  <input 
                    ...
                    class="mr-2"
                  >
                  <span class="rent-column text-sm text-gray-700">{{ hall.name }}</span>
                </label>
              </div>
            </div>
              ...
              <div v-else class="space-y-2 max-h-40 overflow-y-auto border border-gray-400 rounded-lg p-3">
                <label v-for="guesthouse in guesthousesList" :key="guesthouse.id" class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                  <input 
                    ...
                    class="mr-2"
                  >
                  <span class="rent-column text-sm text-gray-700">{{ guesthouse.name }}</span>
                </label>
              </div>
            </div>
          </div>
...
import vipIcon from '@/assets/vipclass.png';
import standardIcon from '@/assets/standardclass.png';
...
export default {
  ...
  methods: {
    ...
    /**
     * Get room type icon path
     * @param {String} type - The room type (vip/standard)
     * @returns {String|null} - The icon path or null
     */
    getRoomTypeIcon(type) {
      ...
      try {
        if (type.toLowerCase() === 'vip') {
          return require('@/assets/vipclass.png');
        } else if (type.toLowerCase() === 'standard') {
          return require('@/assets/standardclass.png');
        }
      } catch (error) {
        console.warn('Icon not found for room type:', type);
        return null;
      }
      return null;
    },
