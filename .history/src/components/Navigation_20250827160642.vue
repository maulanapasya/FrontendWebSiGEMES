  ...
  <header class="navbar flex justify-between items-center px-6 py-3 shadow-md">
    ...
    <div class="relative">
      <button
        ...
        class="bg-white border rounded-lg shadow-sm flex items-center gap-2 px-3 py-2 hover:bg-gray-50 text-black"
      >
        <!-- Ikon user (outline stabil) -->
        <svg viewBox="0 0 24 24" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2">
          <circle cx="12" cy="7" r="4"></circle>
          <path d="M20 21a8 8 0 10-16 0" stroke-linecap="round" stroke-linejoin="round"></path>
        </svg>
      ...
      <!-- Dropdown Menu -->
      <div v-if="dropdownOpen" class="absolute right-0 mt-2 w-40 bg-white border rounded-lg shadow-md dropdown-menu">
        <ul class="py-2">
          <li>
            <router-link 
              to="/profile" 
              ...
              @click="dropdownOpen = false v-if="isSuperAdmin"">
              Profile
            </router-link>
          </li>
          <li>
